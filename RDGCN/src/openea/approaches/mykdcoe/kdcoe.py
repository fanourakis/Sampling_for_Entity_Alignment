...
class KDCoE(BasicModel):
    ...
    def _get_desc_input(self):
        ...
        list1 = self.kgs.train_entities1 + \
            self.kgs.valid_entities1 + self.kgs.test_entities1
        list2 = self.kgs.train_entities2 + \
            self.kgs.valid_entities2 + self.kgs.test_entities2
        ...
        2  20874   38                                              99657
        """
        aid1 = pd.Series(list(model.kgs.kg1.attributes_id_dict),
                         index=model.kgs.kg1.attributes_id_dict.values())
        aid2 = pd.Series(list(model.kgs.kg2.attributes_id_dict),
                         index=model.kgs.kg2.attributes_id_dict.values())
        """
        ...
        """
        # uri_name = 'escription'   in Wikidata, the attribute is
        # http://schema.org/description
        print("name")
        uri_name = 'name'
        desc_uris1 = aid1[aid1.str.findall(
            uri_name).apply(lambda x: len(x)) > 0]
        desc_uris2 = aid2[aid2.str.findall(
            uri_name).apply(lambda x: len(x)) > 0]
        """
        ...
        print("kg1 descriptions:", e_desc1)
        print("kg2 descriptions:", e_desc2)
        """
        ...
        print("after drop_duplicates, kg1 descriptions:", e_desc1)
        print("after drop_duplicates, kg2 descriptions:", e_desc2)
        ...
        # prepare desc
        e_desc1.iloc[:, 2] = e_desc1.iloc[:, 2].str.replace(
            r'[{}]+'.format(string.punctuation), '').str.split(' ')
        e_desc2.iloc[:, 2] = e_desc2.iloc[:, 2].str.replace(
            r'[{}]+'.format(string.punctuation), '').str.split(' ')
        """
        ...
        names.iloc[:, 2] = names.iloc[:, 2].str.replace(
            r'[{}]+'.format(string.punctuation), '').str.split(' ')
        names.iloc[e_desc1.iloc[:, 0].values, [
            1, 2]] = e_desc1.iloc[:, [1, 2]].values
        names.iloc[e_desc2.iloc[:, 0].values, [
            1, 2]] = e_desc2.iloc[:, [1, 2]].values
        """
        ...
        def lookup_and_padding(x):
            default_length = 4
            ids = list(all_word.loc[x].values) + \
                [all_word.iloc[-1], ] * default_length
        ...
        e_desc_input.iloc[names.iloc[:, 0].values] = np.stack(names.iloc[
                                                              :, 2].values)

        print('generating desc input costs time: {:.4f}s'.format(
            time.time() - start))
    ...
    def _get_local_name_by_name_triple(self, name_attribute_list=None):
        if name_attribute_list is None:
            if 'D_Y' in self.args.training_data:
                ...
                name_attribute_list = {
                    'http://www.wikidata.org/entity/P373', 'http://www.wikidata.org/entity/P1476'}
            else:
                name_attribute_list = {}
    ...
    def _define_embed_graph(self):
        ...
        with tf.name_scope('triple_loss'):
            self.triple_loss = margin_loss(
                phs, prs, pts, nhs, nrs, nts, self.args.margin, self.args.loss_norm)
    ...
    def _define_desc_graph(self):
        with tf.variable_scope('desc'):
            self.desc1 = AM_desc1_batch = tf.placeholder(dtype=tf.float32,
                                                         shape=[
                                                             None, self.default_desc_length, self.wv_dim],
                                                         name='desc1')
            ...
            gru_1 = tf.contrib.keras.layers.GRU(
                units=self.wv_dim, return_sequences=True)
            ...
            ds3 = tf.contrib.keras.layers.Dense(
                units=self.wv_dim, activation=tf.tanh, use_bias=True)

            self._att1 = att1 = tf.contrib.keras.layers.Dense(
                units=1, activation='tanh', use_bias=True)
            self._att3 = att3 = tf.contrib.keras.layers.Dense(
                units=1, activation='tanh', use_bias=True)
            ...
            # tf.nn.l2_normalize(DS4(ds2_b), dim=1)
            eb_desc_batch2 = tf.nn.l2_normalize(ds3(ds2_b), dim=1)

            indicator = np.empty(
                (self.desc_batch_size, self.desc_batch_size), dtype=np.float32)
            ...
            self.desc_optimizer = get_optimizer(
                self.args.optimizer, self.args.learning_rate).minimize(self.desc_loss)
    ...
    def _define_mapping_graph_new(self):
        ...
        with tf.name_scope('seed_links_lookup_new'):
            tes1 = tf.nn.embedding_lookup(
                self.ent_embeds, self.seed_entities1_new)
            tes2 = tf.nn.embedding_lookup(
                self.ent_embeds, self.seed_entities2_new)
        with tf.name_scope('mapping_loss_new'):
            self.mapping_loss_new = self.args.new_param * \
                mapping_loss(tes1, tes2, self.mapping_mat, self.eye_mat)
    ...
    def find_new_alignment_desc(self):
        ...
        to_align_links = pd.DataFrame(
            self.kgs.valid_links + self.kgs.test_links)
        ...
        search_tasks = task_divide(
            np.array(range(num)), self.args.test_threads_num // 2)

        pool = mp.Pool(processes=len(search_tasks))
        rests = list()
        for indexes in search_tasks:
            sub_embeds = dem1[indexes, :]
            rests.append(pool.apply_async(find_alignment,
                                          (sub_embeds, dem2, indexes, self.desc_sim_th)))
        pool.close()
        pool.join()
        ...
        if len(self.new_alignment_index) == 0:
            ...
            self.new_alignment_index |= set(new_alignment_desc_index)
            stop = False
        check_new_alignment(self.new_alignment_index,
                            context='check total new alignment')
        self.new_alignment = [(un_aligned_ent1[x], un_aligned_ent2[y])
                              for (x, y) in self.new_alignment_index]
        # del desc_em1, desc_em2, dem1, dem2, desc_sim
        print('finding new alignment costs time: {:.4f}s'.format(
            time.time() - t))
        return stop
    ...
    def find_new_alignment_rel(self):
        ...
        embeds1 = tf.nn.embedding_lookup(
            self.ent_embeds, un_aligned_ent1).eval(session=self.session)
        embeds2 = tf.nn.embedding_lookup(
            self.ent_embeds, un_aligned_ent2).eval(session=self.session)
    ...
    def launch_mapping_training_1epo(self, epoch, triple_steps):
        ...
        print('epoch {}, avg. mapping loss: {:.4f}, cost time: {:.4f}s'.format(
    ...
    def launch_mapping_training_1epo_new(self, epoch, triple_steps):
        ...
        for i in range(triple_steps):
            ...
            trained_samples_num += len(links_batch)
        epoch_loss /= trained_samples_num
        print('epoch {}, avg. new mapping loss: {:.4f}, cost time: {:.4f}s'.format(
    ...
    def launch_desc_1epo(self, epoch):
        ...
        if len(self.new_alignment) > 0:
            alignments = pd.DataFrame(
                self.kgs.train_links + list(self.new_alignment))
        else:
        ...
        choices = np.random.choice(
            len(alignments), size=len(alignments), replace=True)
        epoch_loss = 0.0
        ...
        print('epoch {}, avg. desc loss: {:.4f}, cost time: {:.4f}s'.format(
            epoch, epoch_loss, time.time() - start))
    ...
    def run(self):
        ...
        triples_num = self.kgs.kg1.relation_triples_num + \
            self.kgs.kg2.relation_triples_num
        triple_steps = int(math.ceil(triples_num / self.args.batch_size))
        steps_tasks = task_divide(
            list(range(triple_steps)), self.args.batch_threads_num)
        manager = mp.Manager()
        ...
        for it in range(1, self.args.max_iter + 1):
            ...
            self.early_stop = False
            for i in range(1, self.args.max_epoch + 1):
                self.launch_triple_training_1epo(
                    i, triple_steps, steps_tasks, training_batch_queue, None, None)
