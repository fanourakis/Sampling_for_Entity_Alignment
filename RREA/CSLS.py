...
def cal_rank_by_sim_mat(task, sim, top_k, accurate, test_ent_list):
    mean = 0
    mrr = 0
    ...
    for i in range(len(task)):
        num_for_bias = [0 for k in top_k]
        ref = task[i]
        if accurate:
            rank = (-sim[i, :]).argsort()
            if len(test_ent_list) > 0:
                neighbors[(ref,test_ent_list[ref])] = list(rank)
        else:
        ...
        for j in range(len(top_k)):
            if rank_index < top_k[j]:
                num[j] += 1
                num_for_bias[j] += 1
        ent_metr_list.append((ref, num_for_bias))
    return mean, mrr, num, prec_set, ent_metr_list, neighbors


def eval_alignment_by_sim_mat(embed1, embed2, top_k, nums_threads, dataset, entity1, entity2, csls, accurate=False,output = True, testing=False, iteration = 0, test_ent_list = []):
    t = time.time()
    sim_mat = sim_handler(embed1, embed2, csls, nums_threads)
    ref_num = sim_mat.shape[0]
    neighbors_sim_dict = dict()
    print("ref_num")
    print(ref_num)
    ...
    reses = list()
    for task in tasks:
        reses.append(pool.apply_async(cal_rank_by_sim_mat, (task, sim_mat[task, :], top_k, accurate, test_ent_list)))
    pool.close()
    pool.join()

    for res in reses:

        mean, mrr, num, prec_set, ent_metr_list, neighbors = res.get()
        t_mean += mean
        t_mrr += mrr
        t_num += np.array(num)
        t_prec_set |= prec_set
        for p1, p2 in neighbors:
            neighbors_sim_dict[(p1,p2)] = neighbors[(p1,p2)]
    ...
    performance = dict()
    performance["acc"] = acc
    performance["mr"] = t_mean
    performance["mrr"] = t_mrr
    del sim_mat
    gc.collect()
    return t_prec_set, hits1, performance, neighbors_sim_dict
...
def generate_neighbours_similarities(entity_embeds1, entity_list1, entity_embeds2, entity_list2, neighbors_num, threads_num=4, csls=0):
    ...
    for i in range(len(ent_frags)):
        sim_mat = sim_handler(np.array([entity_embeds1[e] for e in ent_frag_indexes[i]]), np.array(entity_embeds2), csls, 4)
        results.append(pool.apply_async(find_neighbours,
                                        args=(sim_mat, ent_frags[i],
                                              np.array([entity_embeds1[e] for e in ent_frag_indexes[i]]),
                                              list(entity_list2),
                                              np.array(entity_embeds2),
                                              neighbors_num)))
    pool.close()
    pool.join()
...
def find_neighbours(sim_mat, frags, sub_embed1, entity_list2, embed2, k):
    dic = dict()
    # replaced with sim_mat by sim_handler
    # sim_mat = np.matmul(sub_embed1, embed2.T)
    for i in range(sim_mat.shape[0]):
        # todo argsort or argpartition
        sort_index = np.argsort(-sim_mat[i, :])
        neighbors_index = sort_index[0:k]
        neighbors_sim = sim_mat[i, neighbors_index]
        neighbors = list()
        counter = 0
        for f in neighbors_index:
            neighbors.append((entity_list2[f], neighbors_sim[counter]))
            counter += 1
